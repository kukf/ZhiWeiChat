...
import com.doohaa.chat.volley.multipart.BodyWritable;
...
public class HttpUrlStack implements HttpStack {
	...
	private static void addBodyIfExists(HttpURLConnection connection, Request<?> request)
			throws IOException, AuthFailureError {
		if (request instanceof BodyWritable) {
			connection.setDoOutput(true);
			connection.addRequestProperty(HEADER_CONTENT_TYPE, request.getBodyContentType());
			connection.setFixedLengthStreamingMode(((BodyWritable)request).getFixedLength());
			DataOutputStream out = new DataOutputStream(connection.getOutputStream());
			((BodyWritable)request).writeBody(out);
			out.close();
		} else {
