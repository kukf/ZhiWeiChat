...
package com.doohaa.chat.widget.imagecrop;
...
class HighlightView {
	...
	private void init() {
		...
		mResizeDrawableWidth = resources.getDrawable(R.drawable.q_icon_profile_crop_02);
		mResizeDrawableHeight = resources.getDrawable(R.drawable.q_icon_profile_crop);
		mResizeDrawableDiagonal = resources.getDrawable(R.drawable.q_icon_profile_crop_circle);
	}
	...
	protected void draw(Canvas canvas) {
		...
		if (!hasFocus()) {
			...
			if (mCircle) {
				...
				path.addCircle(mDrawRect.left + (width / 2), mDrawRect.top + (height / 2), width
					/ 2, Path.Direction.CW);
				//mOutlinePaint.setColor(0xFF36ff00);
				mOutlinePaint.setColor(mContext.getResources().getColor(R.color.bg_black_60));
				...
				// add RectLine (white)
				circlePulsRectLine(canvas, viewDrawingRect);
			...
			// Move일때는 화살표없애고 나머지는 화살표를 표시해준다.
			...
			// ModifyMode.Grow일때 circle일 경우와 아닐경우 화살표 표시 삭제
		}
	}

	private void circlePulsRectLine(Canvas canvas, Rect viewDrawingRect) {
		Path pathOutRect = new Path();
		Rect topRect = new Rect(viewDrawingRect.left, viewDrawingRect.top, viewDrawingRect.right, mDrawRect.top);
		if (topRect.width() > 0 && topRect.height() > 0) {
			canvas.drawRect(topRect, hasFocus() ? mFocusPaint : mNoFocusPaint);
		}
		...
		pathOutRect.addRect(new RectF(mDrawRect), Path.Direction.CW);

		mOutlinePaintOfRect.setColor(0xFFffffff);
		canvas.drawPath(pathOutRect, mOutlinePaintOfRect);
	}
	...
	public int getHitNoCircle(float x, float y) {

		Rect r = computeLayout();
		final float hysteresis = 40F; // 터치 선택영역 크기
	...
	void moveBy(float dx, float dy) {
		...
		mCropRect.offset(Math.min(0, mImageRect.right - mCropRect.right), Math.min(0, mImageRect.bottom
			- mCropRect.bottom));
	...
	void growBy(float dx, float dy) {
		...
		if (r.width() <= MIN_CIRCLE_SIZE || r.height() <= MIN_CIRCLE_SIZE) {
			return;
		}
		mCropRect.set(r);
		mDrawRect = computeLayout();
	...
	// https://github.com/hanihashemi/android-crop/commit/f7bd04674df928557c75949ef9107eb1c350736b
	public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean circle, boolean maintainAspectRatio, int minX){
		this.minX = minX;
		setup(m,imageRect, cropRect, circle, maintainAspectRatio);
	}
	...
	public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean circle, boolean maintainAspectRatio) {
		...
		//mOutlinePaint.setStrokeWidth(ScreenUtil.dp2px(3.33f));
		mOutlinePaint.setStyle(Paint.Style.STROKE);
		mOutlinePaint.setAntiAlias(true);

		mOutlinePaintOfRect.setStyle(Paint.Style.STROKE);
		mOutlinePaintOfRect.setAntiAlias(true);
	...
	private final Paint mOutlinePaintOfRect = new Paint();
}
